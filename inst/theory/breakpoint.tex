...
where \((\beta,\gamma,\psi) \in \Real^3\), \(\varepsilon
\sim \Gaus[0,\sigma^2]\), and \((x)_+=x\) if \(x>0\) and 0 otherwise.

\bigskip

Denote by \(\Theta = (\beta,\gamma,\psi,\sigma^2)\), we can
express the likelihood relative to \(n\) iid observations as:
\begin{align*}
\Likelihood(\Theta) = \prod_{i=1} \frac{1}{\sqrt{2\pi\sigma^2}}\exp\left(-\frac{(Y_i - \beta X_i - \gamma (X_i - \psi)_+)^2}{2\sigma^2}\right)
\end{align*}
So maximizing the likelihood with respect to \(\Theta_\mu =
(\beta,\gamma,\psi)\) is equivalent to minimizing the mean
squared error:
\begin{align*}
\likelihood(\Theta_\mu) = \sum_{i=1} (Y_i - \beta X_i - \gamma (X_i - \psi)_+)^2 \\
\end{align*}
