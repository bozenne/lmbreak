...
\section{No covariate, no plateau}
\label{sec:org750df01}

Consider:
\begin{itemize}
\item a response variable \(Y\)
\item a explanatory variable \(X\) taking values in the interval
\([x_{\min};x_{\max}]\).
\end{itemize}

\bigskip

\noindent Solving \(a+b x_{\min}=-1\) and \(a+b x_{\max}=-1\) leads to
  \(b = \frac{2}{x_{\max}-x_{\min}}\) and \(a = -
  \frac{x_{\max}+x_{\min}}{x_{\max}-x_{\min}}\). \newline Therefore
  the transformation \(f(.):=\tanh^{-1}(a+b.)\) moves from the
  constrained to the unconstrained scaled. We will thus consider the
  following broken line model for the mean:
\begin{align*}
\Esp[Y] &= \alpha + \beta_1 X + \beta_2 (X-\psi)_{+} \\
&= \alpha + \beta_1 X + \beta_2 (X-\tanh^{-1}(a+b \phi))_{+}
 \end{align*}

We have that at the true breakpoint value \(\psi_0\) and for \(X \neq \psi_0\)
\begin{align*}
(X-\psi)_{+} = f(\psi) = f(\psi_0) + (\psi-\psi_0)f'(\psi_0) = (X-\psi_0)_{+}  - (\psi-\psi_0)\Ind[X>\psi_0]
\end{align*}
Now with transformation:
\begin{align*}
(X-\tanh^{-1}(a+b \phi))_{+} = f(\phi) = f(\phi_0) + (\phi-\phi_0)f'(\phi_0) = (X-\psi_0)_{+}  - (\phi-\phi_0)\frac{b\Ind[X>\psi_0]}{1-\psi_0^2}
\end{align*}
\end{document}